...
df = pd.read_csv("../Dataset/21-02-2018(Time).csv", low_memory = False)
...
for i, (train, test) in enumerate(skf.split(input_label, output_label)):
    ...
    model = createModel()
    model.fit(x = np.array(inp_train), y = np.array(out_train), validation_split= 0.1, shuffle = True, epochs = 10,verbose = 2)
